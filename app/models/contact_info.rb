class ContactInfo < ActiveRecord::Base
  ...
  def get_user_current_address
    # address = [self.npa_localite.scan(/\d+/).map(&:to_i).first, Country[self.pays].name].join(' ')
    # address
    # "#{self.current_postcode} #{self.current_town} #{self.current_country}"
    "#{self.current_address.squish} #{self.current_town} #{self.current_postcode} #{self.current_country}"
  end
